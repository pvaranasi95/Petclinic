...
// }
    ...
    steps {
        bat """
            jf rt u "*%BUILD_NUMBER%.*" "http://localhost:8082/artifactory/%JOB_NAME%/%BUILD_NUMBER%/" --user=%ARTIFACTORY_CRED_USR% --password=%ARTIFACTORY_CRED_PSW%  --url=http://localhost:8082/artifactory/
        """
    }
}
